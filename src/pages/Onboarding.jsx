...
import { ArrowLeft, Edit, MessageCircle, TrendingUp, LogOut } from "lucide-react";
  ...
  return (
    <div className="p-4 space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <Button variant="ghost" size="icon" onClick={handleLogout} className="text-green-700">
      ...
      <Card className="ayur-card border-0 shadow-lg rounded-2xl">
        <CardContent className="p-6">
          <div className="text-center space-y-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-green-900">{user?.full_name || 'Tara'}</h2>
              <p className="text-sm text-green-600">{user?.email}</p>
            </div>

            <div className="grid grid-cols-3 gap-2 text-sm text-green-800 pt-2">
              <div className="font-medium">Age: {user?.age || '25'}</div>
              <div className="font-medium">Weight: {user?.weight || '72'}kg</div>
              <div className="font-medium">Height: {user?.height || '160'}cm</div>
            </div>

            <Badge className="bg-green-100 text-green-800 border-green-200 py-1 px-3 text-sm">
              Dominant Dosha: {user?.prakriti?.dominant_dosha?.replace('-', '–')?.toUpperCase() || 'VATA–PITTA'}
            </Badge>
            ...
            <div className="flex space-x-3 w-full pt-4">
              ...
              <Button variant="outline" className="flex-1 border-green-300 text-green-700 hover:bg-green-50 rounded-xl font-semibold">
      ...
      <Card className="ayur-card border-0 shadow-lg rounded-2xl">
        <CardHeader>
          <CardTitle className="text-lg font-bold text-green-900">Diet History</CardTitle>
        </CardHeader>
      ...
      <Card className="ayur-card border-0 shadow-lg rounded-2xl">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            <Badge className="bg-green-100 text-green-800 justify-center">Balanced Vata</Badge>
            <Badge className="bg-red-100 text-red-800 justify-center">High Pitta</Badge>
            <Badge className="bg-blue-100 text-blue-800 justify-center">Low Kapha</Badge>
          </div>
        </CardContent>
      </Card>
    </div>
